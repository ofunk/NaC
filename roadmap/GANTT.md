...
| B | Installierbare Skills und deterministische Python-Arbeitsabläufe | Aktiv | 45% | Das erste ausführbare KG-Runtime-Paket und die CLI sind mit Unit-Tests implementiert; `START_HERE` ist der operative Einstieg getrennt vom README-Überblick; Startprüfungen haben Profile für Basis-, Plugin-Dev- und Notariats-Arbeitsplatz-Setups; README-/Index-Referenzen haben klickbare-Link-Validierung; PDF-Export bleibt im aktiven Aufbau manuell; `fertig` bedeutet Merge nach `main` plus sauberer lokaler `main`; Sprachparität blockiert kopierte lokalisierte Markdown-/Textspiegel, prüft SKILL.md-Sprachmarker und erzwingt echte Umlaute in deutscher Menschensprache; der KG-Editor stellt sichere No-code-Formular-/Checklisten-Sichten plus Patch-Vertrag bereit; der BPMN-js Business Layer hat ein NaC-Modellprofil, einen ersten Immobilienkaufvertrag-Prozess und eine Python-Validierung im strikten Quality Gate. |
